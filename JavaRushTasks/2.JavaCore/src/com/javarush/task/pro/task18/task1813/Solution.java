...
Для этих целей есть метод onlyElectricCars(ArrayList<Car>), который фильтрует список автомобилей,
 получаемых в качестве входящего параметра. Этот метод возвращает поток отфильтрованных данных, а именно — поток автомобилей, у которых поле isElectric равно true. В этом тебе поможет метод filter() объекта типа Stream<Car>.
*/
...
public class Solution {
    ...
    public static Stream<Car> onlyElectricCars(ArrayList<Car> cars) {
        Stream<Car>stream = cars.stream().filter(s -> s.isElectric());
//        Stream<Car>stream1 = cars.stream().filter(Car::isElectric);
        return stream;
    }
}
